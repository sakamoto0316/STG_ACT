...
//静的メンバ変数宣言
bool CManager::m_bEdit = false;
bool CManager::m_bStop = false;
bool CManager::m_SetTutorial = false;
bool CManager::m_Pause = false;
bool CManager::m_SetScoreResult = false;
bool CManager::m_SetJoyPad = false;
bool CManager::m_PauseOK = false;
int CManager::m_EndScore = 0;
int CManager::m_EndCombo = 0;
int CManager::m_EndFall = 0;
int CManager::m_EndDepth = 0;
...
CLevelUP *CManager::m_LevelUP = NULL;
...
CScene *CScene::Create(MODE mode)
{
	...
	CManager::GetCamera()->ResetCamera();

	CManager::SetEdit(false);

	m_mode = mode;

	if (m_mode == MODE_GAME)
	{
		CGame::SetTutorialStart(CManager::GetTutorialStart());
	}
...
{
	CScene *m_pScene = CManager::GetScene();
